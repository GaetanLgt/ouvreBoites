...
function smallenvelop_widgets_init()
{
    register_sidebar([
        ...
        'before_widget' => '<aside id="%1$s" class="widget col m6 %2$s">',
        'after_widget' => '</aside>',
...
function wporg_custom_post_type()
{
    register_post_type('galerie', [
        'labels' => [
            'name' => __('Galerie', 'textdomain'),
            'singular_name' => __('image', 'textdomain'),
        ],
        'public' => true,
        'has_archive' => true,
    ...
    register_post_type('evenements', [
        'labels' => [
            'name' => __('Evènements', 'textdomain'),
            'singular_name' => __('event', 'textdomain'),
        ],
        'public' => true,
        'has_archive' => true,
        //'rewrite' => ['slug' => 'events'], // my custom slug
        'menu_icon' => 'dashicons-smiley',
    ]);
    register_post_type('poles', [
        'labels' => [
            'name' => __('Pôles', 'textdomain'),
            'singular_name' => __('Pôle', 'textdomain'),
        ],
        'public' => true,
        'has_archive' => true,
        //'rewrite' => ['slug' => 'events'], // my custom slug
...
function my_pre_get_posts($query)
{
    ...
    if (
        ...
        'evenements' == $query->query_vars['post_type']
    ) {
        $query->set('orderby', 'meta_value');
        $query->set('meta_key', 'dateDebut');
...
add_action('init', 'wpshout_add_taxonomies_to_galerie');
function wpshout_add_taxonomies_to_galerie()
{
    register_taxonomy_for_object_type('category', 'galerie');
}
...
function query_post_type($query)
{
    if (is_category()) {
        ...
        if ($post_type) {
            ...
            $post_type = ['nav_menu_item', 'post', 'galerie'];
...
function cpt_archive_posts_per_page($query)
{
    ...
    if (
        $query->is_main_query() &&
        !is_admin() &&
        is_post_type_archive('galerie')
    ) {
        $query->set('posts_per_page', '3'); // number has to be the same like in your query / 'posts_per_page'
...
function custom_admin_scripts()
{
    wp_enqueue_script(
        'acf-pro-input',
        get_template_directory_uri().'/js/acf-pro-input.min.js',
        [],
        false,
        true
    );
    wp_enqueue_script(
        'acf-pro-group',
        get_template_directory_uri().'/js/acf-pro-field-group.min.js',
        [],
        false,
        true
    );
...
<div <?php acf_esc_attr_e($div); ?>>
    ...
    <table class="acf-table">
        ...
        <tbody>
            ...
            <tr class="acf-row<?php if ('acfcloneindex' === $i) {
                      echo ' acf-clone';
                  } ?>" data-id="<?php echo $id; ?>">
                ...
                <td class="acf-row-handle order" title="<?php _e('Drag to reorder', 'acf'); ?>">
                    ...
                    <a class="acf-icon -collapse small" href="#" data-event="collapse-row"
                        title="<?php _e('Click to toggle', 'acf'); ?>"></a>
                    <?php } ?>
                ...
                <td class="acf-row-handle remove">
                    <a class="acf-icon -plus small acf-js-tooltip hide-on-shift" href="#" data-event="add-row"
                        title="<?php _e('Add row', 'acf'); ?>"></a>
                    <a class="acf-icon -duplicate small acf-js-tooltip show-on-shift" href="#"
                        data-event="duplicate-row" title="<?php _e('Duplicate row', 'acf'); ?>"></a>
    ...
    <div class="acf-actions">
        <a class="acf-button button button-primary" href="#"
            data-event="add-row"><?php echo $field['button_label']; ?></a>
    </div>
...
<?php
          ...
          */
          ...
          public function validate_value($valid, $value, $field, $input)
          {
              ...
              if ($min && $count < $min) {
                  ...
                  return str_replace('{min}', $min, $error);
                  // return
              }
          ...
          */
          ...
          public function update_row($row, $i = 0, $field, $post_id)
          {
          ...
          */
          ...
          public function delete_row($i = 0, $field, $post_id)
          {
              // bail early if no sub fields
              if (empty($field['sub_fields'])) {
                  return false;
